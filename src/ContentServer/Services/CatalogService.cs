...
namespace Nexa.ContentServer.Services
{
    ...
    public class CatalogService
    {
        ...
        // Definicje kluczy w Cache Redis-a
        ...
        /// <summary>
        /// Konstruktor - przyjmuje ścieżkę do storage, logger i bazy Redis.
        /// </summary>
        ...
        /// Każdy podfolder to jeden film z metadata.json.
        /// Najpierw sprawdza cache w Redis, jesli nie ma to czyta dane z dysku.
        /// </summary>
        public async Task<List<ContentMetadata>> GetAllContentAsync()
        {
            // Próbuje pobrać dane z cache
            try
            {
            ...
            {
                // Jeśli Redis nie działa, to log i odczyt z dysku
                _logger.LogWarning(ex, "Failed to access Redis cache for {CacheKey}. Falling back to disk.", CacheKeyAllContent);
            }

            _logger.LogInformation("Cache MISS for key: {CacheKey}. Fetching from disk.", CacheKeyAllContent);

            // pobiera z dysku
            var contentList = await FetchAllContentFromDiskAsync();
            ...
            {
                _logger.LogWarning(ex, "Failed to store data in Redis cache for key {CacheKey}", CacheKeyAllContent);
            }
        ...
        /// Najpierw sprawdza cache w Redis
        /// </summary>
        public async Task<ContentMetadata> GetContentByIdAsync(string contentId)
        {
            ...
            {
                _logger.LogWarning(ex, "Failed to access Redis cache for {CacheKey}. Falling back to disk.", cacheKey);
            }

            _logger.LogInformation("Cache MISS for key: {CacheKey}. Fetching from disk.", cacheKey);

            // pobiera z dysku
            var metadata = await FetchContentByIdFromDiskAsync(contentId);
            ...
            {
                _logger.LogWarning(ex, "Failed to store data in Redis cache for key {CacheKey}", cacheKey);
            }

            return metadata;
        }

        // Prywatne metody z oryginalną logiką (przed cache Redis)
        private async Task<List<ContentMetadata>> FetchAllContentFromDiskAsync()
        {
            ...
            try
            {
                ...
                foreach (var contentDir in contentDirectories)
                {
        ...
        private async Task<ContentMetadata> FetchContentByIdFromDiskAsync(string contentId)
        {
